...
import { EncuestasService, Encuesta, JuegoAPI } from '../../services/encuestas.service';
...
export class EncuestaFormPage {
  ...
  enc: Encuesta = { alias: '', videojuego: '' };
  fechaHoy = new Date().toLocaleDateString('es-EC', { day: '2-digit', month: 'long', year: 'numeric' });
  ...
  async obtenerGPS() {
    try {
      ...
      this.toast('No se pudo obtener la ubicación GPS', 'warning');
    }
  }
  ...
  async tomarFoto(fuente: 'camera' | 'gallery') {
    try {
      ...
      if (!e?.message?.includes('cancelled')) {
        this.toast('No se pudo acceder a la cámara/galería', 'warning');
      }
    }
  }

  async buscarEnAPI() {
    if (!this.enc.videojuego) return;
    this.buscandoApi = true;
    this.juegoAPI = undefined;
    this.apiSinResultado = false;
    try {
      // Intenta RAWG primero; si no hay key, usa FreeToGame
      let resultado = await this.svc.buscarJuego(this.enc.videojuego);
      if (!resultado) {
        resultado = await this.svc.buscarJuegoFreeToPlay(this.enc.videojuego);
      }
      if (resultado) {
        this.juegoAPI = resultado;
        // Guardar datos de API en la encuesta
  ...
  async guardar() {
    if (!this.enc.alias.trim() || !this.enc.videojuego.trim()) {
      const alert = await this.alertCtrl.create({
        header: 'Campos requeridos',
        message: 'El alias y el videojuego favorito son obligatorios.',
        buttons: ['OK']
      });
      await alert.present();
      return;
    ...
    try {
      ...
      if (this.fotoBase64) {
        this.enc.foto_url = await this.svc.subirFoto(this.fotoBase64, `enc_${Date.now()}.jpg`);
      }
      await this.svc.crear(this.enc);
      this.toast('✅ Encuesta guardada', 'success');
      this.router.navigate(['/tabs/encuestas']);
    } catch (e) {
      console.error(e);
      this.toast('❌ Error al guardar. Intenta de nuevo.', 'danger');
    } finally {
